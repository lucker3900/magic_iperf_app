...
一个类似 Magic iPerf 的轻量级 Android 客户端，用来快速发起 TCP/UDP iperf2/iperf3 测速。采用 Jetpack Compose 构建，UI 支持常见参数（地址、端口、时长、UDP 目标带宽、正反向切换）与自定义命令行，并即时展示输出日志。

## 目录结构
- `app/`：Android 应用模块（Compose UI、ViewModel、iperf 运行器）
- `app/src/main/assets/`：放置设备架构对应的 `iperf2`/`iperf3` 可执行文件
- `gradlew` / `gradlew.bat`：Gradle Wrapper 脚本（需要本地 JDK 17+）

## 快速开始
1) 准备 iperf 可执行文件  
   - 将为设备架构编译好的 `iperf3` 重命名为 `iperf3`（无扩展名）后放到 `app/src/main/assets/`  
   - 如需 iperf2，同样放置 `iperf2`（无扩展名）到 assets；UI 中可切换 iperf 版本  
...
3) App 使用  
   ...
   - 或直接在“自定义参数”输入整行命令参数（示例：`-c 192.168.0.100 -i 1 -P 5 -t 60 -b 100M`），会直接附加在 iperf 命令后执行  
   - 切换“正向/反向”按钮以便测试上行或下行  
   - 点击“开始测试”，日志区域会实时显示 iperf 输出；错误会以 Snackbar 提示

## 备注
- 若需要调整 Gradle 版本，可修改 `gradle/wrapper/gradle-wrapper.properties` 中的 `distributionUrl`
- 项目默认禁用混淆，发布前可在 `app/build.gradle.kts` 中开启
- 建议不要将真实 iperf 二进制提交到仓库，以避免仓库过大
